...
本项目可直接用backend/scripts/design_trend_model_adapter.py；命令数组中的python换成实际base环境Python绝对路径。适配器管理连接配置，不把密钥写进Skill或Prompt。每步有效max_tokens写入请求model_profile；用户指定更小上限时不会增加。
