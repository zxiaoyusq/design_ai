...
confirmed 模型观察可省略规则计数及 core/auxiliary 命中数组。宿主先依据 `style-evidence-rules.json` 对规范字段的当前值执行可审计的确定性匹配，只在明确命中时自动挂接证据；字段 ID 位于 allowlist 本身不能证明其任意值支持该风格。值级规则无法安全决定但存在强候选字段时，宿主只发送当前风格规则和少量字段值执行窄范围语义复核，不携带图片、完整 Skill 或完整注册表。低于 0.75、无可用值、无证据或越出 allowlist 的引用只从风格证据链剔除，原设计字段及不确定项继续保留。整理后必须重新检查决定锚点、独立辅助字段、证据数、区域、规则覆盖、颜色、排除项和注册表特殊门槛；只有剩余强证据完整闭环时才能保持 confirmed。必需字段、特殊硬门槛和关系冲突不得通过自动挂接、语义复核或裁剪绕过。
...
模型不输出可由注册表或已有观察确定得到的字段/风格元数据、模块清单、规则计数、排序、confirmed 候选镜像与 core/auxiliary 证据链。宿主使用 `compile_model_output.py` 生成 `design_dna_multitag_extraction_v1.1` 完整结构，并负责字段准入复核、显式受控值归一化、低置信镜像同步、值级证据挂接以及风格证据闭环复核。只有值级规则无法安全决定的少量候选才进入窄范围语义复核；无法闭环的字段置为未知或移除，无法闭环的 confirmed 风格保守降级为 provisional。宿主与复核器均不得补造视觉值或证据。两个 Schema 分别是模型观察和最终字段结构的唯一权威。
