...
`multimodal-design-dna-multitag-extractor` 1.6.2 已完成字段准入、受控值归一化、低置信镜像重建、证据区域声明同步、值级风格证据自动挂接、窄范围语义复核、弱引用裁剪与重新闭环、结构化源路径诊断及完整回归评测。它继续独立维护，不替换或修改原 `multimodal-design-dna-extractor`。
...
- 证据框相对主体框每边不超过 0.03 的坐标取整偏差由宿主吸收；更大的越界不做修正并继续由最终校验拒绝。
...
- 工程后端 57 项自动化测试及前端 TypeScript 生产构建通过。
