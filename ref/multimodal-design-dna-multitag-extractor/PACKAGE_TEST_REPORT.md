...
`multimodal-design-dna-multitag-extractor` 1.1.0 已完成风格扩展、确定性校验和回归评测。它继续独立维护，不替换或修改原 `multimodal-design-dna-extractor`。
...
- 12 个命名组合预设只把已确认原子标签转换为查询条件；预设 ID 与活动标签不重名，不进入输出，也不参与反向补证。
...
- 80 条 JSONL 评测 ID 唯一且可解析：24 条混淆、19 条通用、5 条多标签、32 条扩展风格正例/边界/反例/共存例；仅 `MT-01`～`MT-05` 自然触发本 Skill。
- 变异门禁可拒绝旧层级字段、废弃标签、低置信风格字段、零规则计数、dominance/排序错误、候选状态错配、缺失标签对、同区冲突、同证据换字段、非法区域及非有限数值；合法的同区独立纹饰共存与冲突降级候选可通过。
- 知识索引重建一致；Prompt bundle 可完整生成，共 200,451 字符、255,127 字节，SHA-256 为 `5902384ac25ca9775822c9f4805282025fb5e0ba415b5938da11c45b979ad0fd`。
- 包级结构、版本、Schema、示例、评测、脚本、索引及 31 个内容文件校验和通过。
