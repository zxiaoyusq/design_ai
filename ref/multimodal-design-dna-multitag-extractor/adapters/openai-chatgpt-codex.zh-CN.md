...
- 有结构化输出能力时使用 `schemas/design-dna-model-output.schema.json`；最终 Schema 由宿主后处理校验。
- 生成后运行 `scripts/validate_output.py`；Agent 只返回 JSON，保存与业务视图转换由宿主负责。
- 分析时加载的知识库版本必须与结果中的 `knowledge_base_version` 一致。
- 当前有 38 个活动标签；TribeIdentity 已废弃，身份信息只记录为 IDG-05/07/09。不要读取或生成组合预设，宿主 Python 会在模型输出后写入。
