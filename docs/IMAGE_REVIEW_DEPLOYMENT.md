...
> 发布状态：用户已授权公开上传；本版附件不加密，无需密码。版本地址：https://github.com/zxiaoyusq/design_ai/releases/tag/image-review-20260921
...
本版附件未加密，恢复时不需要密码或 OpenSSL。恢复脚本会校验所有附件和文件哈希。

下载可重试，已经通过 SHA-256 校验的分卷会复用。下载缓存位于 `.runtime/image-review-bundles/image-review-20260921`。解包完成前需要约 5 GB 空闲空间（压缩分卷及恢复的图片）。已存在同名任务时脚本拒绝覆盖，避免覆盖服务器上的人工选择。使用原有方式重启站点，例如由 systemd 托管时重启 `design-ai-site.service`；直接运行时使用 `./site.sh restart`。
...
- 结果图片：396 张，其中保留 385 张（revision 42）；其他图片：6,592 张，暂未保留（revision 4），已经排除任一用户 DISLIKE 的 1,235 张图片。打包时的选择版本和保留数见 `deploy/image-review/manifest.json`。
