...
- 证据框相对主体框每边不超过 0.03 的差异视为坐标取整偏差，宿主可把主体框扩至证据边界；任一边超过容差时不得自动扩大。
...
- profile、必要视角、静态元数据、统计、排序和不确定项镜像继续由宿主负责；显式别名、关系描述中的标准关系词和规定离散刻度可确定性归一化，不能无损归一化的值只能转为未知或移除，不能发明替代视觉事实。
